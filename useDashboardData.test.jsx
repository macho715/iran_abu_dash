...
vi.mock("../lib/livePointer.js", () => ({
  fetchLatestPointer: pointerMocks.fetchLatestPointer,
  fetchPointerArtifact: pointerMocks.fetchPointerArtifact
}));
...
describe("useDashboardData", () => {
  beforeEach(() => {
    pointerMocks.fetchLatestPointer.mockReset();
    pointerMocks.fetchPointerArtifact.mockReset();
    pointerMocks.fetchLatestPointer.mockImplementation(async () => {
      throw new Error("pointer offline");
    });
    pointerMocks.fetchPointerArtifact.mockImplementation(async () => null);
  ...
  it("keeps polling active in React.StrictMode after the first dev-only effect cycle", async () => {
    vi.useFakeTimers();

    pointerMocks.fetchLatestPointer.mockResolvedValue({
      version: "2026-03-06T10-00-00Z",
      liteUrl: "https://example.com/live/v/2026-03-06T10-00-00Z/state-lite.json",
      aiVersion: null,
      aiUrl: null,
    });
    pointerMocks.fetchPointerArtifact.mockResolvedValue({
      dashboard: JSON.parse(JSON.stringify(mockState.cachedPayload.dashboard)),
    });

    const wrapper = ({ children }) => <React.StrictMode>{children}</React.StrictMode>;
    const { unmount } = renderHook(() => useDashboardData(), { wrapper });

    await waitFor(() => expect(pointerMocks.fetchLatestPointer).toHaveBeenCalled());
    const callsAfterMount = pointerMocks.fetchLatestPointer.mock.calls.length;

    await vi.advanceTimersByTimeAsync(31_000);

    await waitFor(() => {
      expect(pointerMocks.fetchLatestPointer.mock.calls.length).toBeGreaterThan(callsAfterMount);
    });
