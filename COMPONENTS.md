...
| AskAiPanel | `react/src/components/AskAiPanel.jsx` | 헤더에서 여는 전역 Ask AI 모달 (단일 Q/A) |
...
| aiConfig | `react/src/lib/aiConfig.js` | AI endpoint/token window·query·env·localStorage 우선순위 해석 |
| aiChat | `react/src/lib/aiChat.js` | direct browser→proxy POST `/api/ai/chat` 호출 및 응답 텍스트 정규화 |
...
| AI 보조 버튼 | action + ui 요약 | 사용자 요청 | `callAiChat()`를 직접 호출하고, 응답은 카드 내부 접이식 영역에만 표시 |

### 4.7 Ask AI 모달 (AskAiPanel)

| 컴포넌트 | 데이터 소스 | 고정/업데이트 | 작동 로직 |
|----------|-------------|---------------|-----------|
| 질문 입력 | 로컬 state | 사용자 입력 | 단일 질문 textarea, 최신 응답 1개만 유지 |
| 컨텍스트 요약 | dash, derived, usableRoutes, summary | 업데이트 | raw 원문 대신 mode/gate/airspace/conflict/top routes/top intel/summary만 전송 |
| AI 응답 | `callAiChat` | 사용자 요청 | 헤더 버튼으로 열고 ESC/배경 클릭/focus trap 지원 |
...
- `DashboardHeader` → `AskAiPanel` 모달 오픈
- `ShortcutsOverlay`와 동일한 overlay 접근성 패턴 재사용
